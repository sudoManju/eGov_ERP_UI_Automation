...
import org.egov.bpa.application.entity.ExistingBuildingDetail;
import org.egov.bpa.application.entity.ExistingBuildingFloorDetail;
import org.egov.bpa.application.repository.ExistingBuildingFloorDetailsRepository;
...
public class ExistingBuildingFloorDetailsService {
    ...
    public void buildExistingBuildingFloorDetails(final BpaApplication application) {
        if (!application.getExistingBuildingDetails().isEmpty()
                && null != application.getExistingBuildingDetails().get(0).getTotalPlintArea()
                && !application.getExistingBuildingDetails().get(0).getExistingBuildingFloorDetails().isEmpty()) {
            buildAndDeleteExistingBuildingFloorDetails(application);
            buildNewlyAddedFloorDetails(application);
            List<ExistingBuildingFloorDetail> floorDetailsList = new ArrayList<>();
            application.getExistingBuildingDetails().get(0).setApplication(application);
            for (ExistingBuildingFloorDetail existingBuildFloorDetails : application.getExistingBuildingDetails().get(0)
                ...
                if (null != existingBuildFloorDetails && null == existingBuildFloorDetails.getId()
                    ...
                    floorDetails.setExistingBuildingDetail(application.getExistingBuildingDetails().get(0));
            ...
            application.getExistingBuildingDetails().get(0).getExistingBuildingFloorDetails().clear();
            application.getExistingBuildingDetails().get(0).setExistingBuildingFloorDetails(floorDetailsList);
        }
    ...
    public void buildNewlyAddedFloorDetails(final BpaApplication application) {
        if (!application.getExistingBuildingDetails().get(0).getExistingBuildingFloorDetailsUpdate().isEmpty()) {
            List<ExistingBuildingFloorDetail> newFloorDetails = new ArrayList<>();
            for (ExistingBuildingFloorDetail applicationFloorDetail : application.getExistingBuildingDetails().get(0)
                    .getExistingBuildingFloorDetailsUpdate()) {
            ...
            application.getExistingBuildingDetails().get(0).getExistingBuildingFloorDetails().addAll(newFloorDetails);
        }
    }

    private void buildAndDeleteExistingBuildingFloorDetails(final BpaApplication application) {
        List<ExistingBuildingFloorDetail> existingFloorDetails = new ArrayList<>();
        if (application.getExistingBuildingDetails().get(0).getDeletedFloorIds() != null
                && application.getExistingBuildingDetails().get(0).getDeletedFloorIds().length > 0) {
            for (Long id : application.getExistingBuildingDetails().get(0).getDeletedFloorIds()) {
