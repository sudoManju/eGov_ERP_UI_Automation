...
public class LoginResource extends Resource {
    ...
    public Response getSessionIdFromPilotBaseAPI() {
        new APILogger().log("Get The SESSION WITH_IDS From Base API Test Is Started -- ");
        Response response = given().request().with()
                .urlEncodingEnabled(false)
                .when()
                .get("http://kurnool-pilot-services.egovernments.org/egi");
        scenarioContext.setSessionId(response.getCookie("SESSIONID"));
        new APILogger().log("Get The SESSION WITH_IDS From Base API Test Is Completed -- ");
        return response;
    }

    public Response loginFromPilotService(String sessionId, Map map) {
        new APILogger().log("Get The SESSION WITH_IDS From LOGIN API Test Is Started -- ");
        Response response = given().request().with()
                .urlEncodingEnabled(false)
                .header("Content-type", "application/x-www-form-urlencoded")
        ...
        scenarioContext.setSessionId(response.getCookie("SESSIONID"));
        new APILogger().log("Get The SESSION WITH_IDS From LOGIN API Test Is Completed -- ");
        return response;
    }
